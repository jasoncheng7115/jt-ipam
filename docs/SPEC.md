...
> 繁體中文版：[SPEC_zh-TW.md](SPEC_zh-TW.md)
...
- **Built-in local AI**: LLM Server, natural-language queries, semantic search
...
├── id
├── server_id → DNSServer
├── name                     # "jason.tools", "168.192.in-addr.arpa"
...
- Forward zones (multiple, e.g. `jason.tools`, `internal.jason.tools`)
...
### 10.4 MCP Server
- exposes IPAM capabilities to an LLM (LLM Server gpt-oss:120b, qwen3.5:122b)
- tools: `search_ip`, `allocate_subnet`, `find_free_ip`, `get_device_by_ip`, `list_vlans`, `trace_mac`, `check_dns_consistency`…
- integrates Jason's existing Telegram Bot + Node-RED + LLM Server
...
- connects to local LLM Server (gpt-oss:120b, qwen3-vl)
...
- ✅ AI semantic search (pgvector + LLM Server embedding)
...
- ✅ local-LLM natural-language query (LLM Server chat + tool use; UI floating window)
...
- Official site: `ipam.jason.tools`
- Demo: `demo-ipam.jason.tools`
- Docs: `docs-ipam.jason.tools`
