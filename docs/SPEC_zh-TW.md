...
- **內建本地 AI 能力**：LLM Server、自然語言查詢、語意搜尋
...
7. **資安內建（Security by Design）**：對齊 OWASP Top 10:2025，詳見 `docs/SECURITY_zh-TW.md`
...
每個 Subnet 可指定：
- 正解 Zone（多個，例：`jason.tools`、`internal.jason.tools`）
...
### 11.1 本地 AI 助手
- 對接本地 LLM Server（gpt-oss:120b、qwen3-vl）
...
### 12.1 資安（總覽，細節見 docs/SECURITY_zh-TW.md）
...
-  本地 LLM 自然語言查詢（LLM Server chat + tool use；UI 浮動視窗）
...
- 官方網站：`ipam.jason.tools`
- Demo：`demo-ipam.jason.tools`
- 文件：`docs-ipam.jason.tools`
